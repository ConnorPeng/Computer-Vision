...
def pairwise_distances(X, Y):
    ...
    for i in range(M):
        temp = X - Y[i]
        D[:,i] = np.linalg.norm(temp, axis=1)
    D = np.array(D)
    #############################################################################
    #                             END OF YOUR CODE
    #############################################################################
...
def kmeans(feature_vectors, k, max_iter=100):
    ...
    #Initialize your centroids with random *unique* points from the input data
    rand = np.random.choice(np.arange(N), size=k, replace=False)
    centroids = feature_vectors[rand]
    
    for i in range(max_iter):
        dist = pairwise_distances(feature_vectors, centroids)
        least_index = np.argmin(dist, axis=1)
        for c in range(k):
            index = feature_vectors[np.where(least_index == c)]
            temp = (len(index) > 0)
            if temp:
...
def build_vocabulary(image_arrays, vocab_size=50, stride=20, max_iter=10):
    ...
    *   The default vocab_size of 50 is sufficient for you to get a
    ...
    for i in image_arrays:
        H, W = i.shape
        float_type = torch.from_numpy(i).type(torch.float32)
        fourD = float_type.view(1, 1, H, W)
        x, y = generate_sample_points(H, W, stride)
        SIFT = get_siftnet_features(fourD, x, y)
        for j in SIFT:
            temp.append(j)
    temp = np.array(temp)
    vocab = kmeans(temp, k = vocab_size, max_iter=max_iter)
    #############################################################################
    #                             END OF YOUR CODE
    #############################################################################
...
def kmeans_quantize(raw_data_pts, centroids):
    ...
    # TODO: YOUR CODE HERE
    #############################################################################
    pair_dist = pairwise_distances(raw_data_pts, centroids)
    indices = np.argmin(pair_dist, axis=1)
    #############################################################################
    #                             END OF YOUR CODE
    #############################################################################
    return indices
...
def get_bags_of_sifts(image_arrays, vocabulary, stride=5):
    ...
    for i in range(num_images):
        img = image_arrays[i]
        H, W = img.shape
        float_type = torch.from_numpy(img).type(torch.float32)
        fourD = float_type.view(1, 1, H, W)
        x, y = generate_sample_points(H, W, stride)
        SIFT = get_siftnet_features(fourD, x, y)
        km = kmeans_quantize(SIFT, vocab)
        hist, bin_e = np.histogram(km, bins = np.arange(vocab_size + 1))
