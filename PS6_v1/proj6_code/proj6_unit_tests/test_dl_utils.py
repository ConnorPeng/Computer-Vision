...
from proj6_code.dl_utils import compute_loss, predict_labels
...
class DummyModel(nn.Module):
    def __init__(self):
        ...
        self.net.weight = nn.Parameter(torch.arange(25, dtype=torch.float32).reshape(5, 5) - 12)
...
def test_compute_loss():
    ...
    assert torch.allclose(
        compute_loss(test_net, test_net(x), torch.LongTensor([3])), torch.FloatTensor([9.500075340270996]), atol=1e-3
    )
